...
def generate_recommendations():
    ...
    if doc.exists and not force_refresh:
        cached_data = doc.to_dict()
        cached_preferences = cached_data.get('preferences', {})
        cached_recommendations = cached_data.get('recommendations', [])
        
        # Compare new preferences with cached preferences
        preferences_changed = any(
            new_preferences.get(key) != cached_preferences.get(key)
            for key in new_preferences
        )
        
        # If preferences haven't changed, return cached recommendations
        if not preferences_changed and cached_recommendations:
            logging.debug("Using cached recommendations - preferences unchanged")
            return jsonify(cached_recommendations), 200
    
    logging.debug("Generating new recommendations - preferences changed or cache invalid")
    
    # Rest of your existing code for generating recommendations
    try:
        ...
        - Desired Vibes: {', '.join(new_preferences['vibes'])}
        - Budget Level: {new_preferences['budget']}
        ...
        store_success = store_user_recommendations(user_id, formatted_recommendations, new_preferences)
